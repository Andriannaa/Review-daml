...
class Dataset(object):
    ...
    def load_reviews(self, max_doc_length, padding_word_id, path):
        ...
        with open(path, "r") as f:
            ...
            while line != None and line != "":
                review = []
                arr = line.split("\t")
                entity = int(arr[0])   # entity为userId号
                word_list = arr[1].split(" ")  # word_list为Id号对应的评论语句word

                for i in xrange(len(word_list)):
                    if (word_list[i] == "" or word_list[i] == None or (not self.word_id_dict.has_key(word_list[i]))):
                        continue
                    review.append(self.word_id_dict.get(word_list[i]))  # 把review中的word用dict中的数字向量表示
    ...
    def load_ratingFile_as_mtrx(self, file_path):   # rating matix
    ...
    def load_ratingFile_as_list(self, file_path):           # rating list
        rateList = []
