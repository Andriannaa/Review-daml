...
def train_model():
    ...
    lamda = lambda_1 * (tf.nn.l2_loss(user_entity_embedding) + tf.nn.l2_loss(item_entity_embedding) + tf.nn.l2_loss(v_entity))
    loss = loss1+lamda
    ...
    with tf.Session() as sess:
        ...
        for e in range(epochs):
            ...
            t2 = time()
            print("epoch%d train time: %.3fs test time: %.3f  loss = %.3f train_mse = %.3f  test_mse = %.3f  test_mae = %.3f"%(e, (t1 - t), (t2 - t1), loss_total/count, all_trainmse/count, mse, mae))
