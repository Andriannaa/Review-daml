...
def train_model():
    ...
    # FM layer
    embeds_sum = tf.concat([user_embeds, item_embeds], 1, name="concat_embed") # [batchsize,2*latent_dim]
    ...
    loss1 = tf.reduce_mean(tf.squared_difference(predict_rating, ratings)) # ratings:[200,1]
    #loss += lambda_1*(tf.nn.l2_loss(W_u)+tf.nn.l2_loss(W_i)+tf.nn.l2_loss(W_u1)+tf.nn.l2_loss(W_i1)+tf.nn.l2_loss(W_u2)+tf.nn.l2_loss(W_i2)+tf.nn.l2_loss(W_u3)+tf.nn.l2_loss(W_i3)+tf.nn.l2_loss(eu_user)+tf.nn.l2_loss(eu_item)+tf.nn.l2_loss(v)+tf.nn.l2_loss(user_bs)+tf.nn.l2_loss(item_bs))
    u_l2 = tf.nn.l2_loss(user_embeds)
    v_l2 = tf.nn.l2_loss(v)
    bs_l2 = tf.nn.l2_loss(user_bs)
    la = tf.nn.l2_loss(user_embeds)+tf.nn.l2_loss(item_embeds)+tf.nn.l2_loss(v)+tf.nn.l2_loss(user_bs)+tf.nn.l2_loss(item_bs)
    loss = loss1+lambda_1*la
    ...
    with tf.Session() as sess:
        ...
        for e in range(epochs):
            ...
            for i in range(int(math.ceil(len(user_input) / float(batch_size)))):   # match.ceil(x)：x的上入整数
                ...
                loss11,u1,v1,b1,lamda,_, loss_val, words,scores,user,item = sess.run([loss1,u_l2,v_l2,bs_l2,la, train_step, loss, word_embeddings,euclidean,eu_user,eu_item],
            ...
            t3 = time()
            print("epoch%d train time: %.3fs  test time: %.3f  loss = %.3f val_mse = %.3f test_mse = %.3f test_mae = %.3f"%(e, (t1 - t), (t3 - t2), loss_total/count, val_mse, mse, mae))
...
if __name__ == "__main__":
    ...
    lambda_1 = 0.005 # 0.01  0.005的时候train mse为正则项的0.1倍
    drop_out = 0.8
    batch_size = 128 # 200
    epochs = 180
    # loading data
